...
const ProfileScreen = () => {
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      try {
        const res = await fetch(`http://192.168.103.19:3000/api/user/${user.id}`, {
          method: "GET",
          headers: {
    ...
    try {
      ...
      if (avatarUrl && avatarUrl !== originalAvatar) {
        ...
        const uploadResp = await fetch(
          `http://192.168.103.19:3000/api/user/${user.id}/avatar`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${token}`,
      ...
      const resp = await fetch(`http://192.168.103.19:3000/api/user/${user.id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
